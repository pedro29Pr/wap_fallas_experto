...
define([
  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    _setBufferInputs: function () {
      ...
      this._checkAndSetLimitsForBufferDistance();
      ...
      this.own(on(this.bufferDistanceUnit, 'change', lang.hitch(this, function () {
        ...
        this._checkAndSetLimitsForBufferDistance();
        if (bufferDistance > 0) {
          //When unit is km enter value 1200, then change unit to miles,
          //it shows invald value in unit as 1000 is max mile value
          //In this case it will now reset the buffer value to max value on unit change as well
          if (!this.bufferDistanceTextBox.isValid()) {
            this._validateBufferDistance(bufferDistance);
          } else {
            this._validateAndAddAOI();
          }
        }
    ...
    /**
     * This function is used to reset buffer distance text-box to zero(if value is -ve/blank/text),
     * or to max limit of current selected unit(if value is exceeding max limit)
     * mouse is clicked elsewhere.
     * @memberOf Screening/Widget
     */
    _validateBufferDistance: function (value) {
      if (isNaN(value) || (!this.bufferDistanceTextBox.isValid() && this.bufferDistanceTextBox < 0)) {
        this.bufferDistanceTextBox.set("value", 0);
      } else {
        this._checkAndSetValueForBufferDistance();
      }
      this._setAriaLabelToBufferDistTextBox();
    },
    ...
    /**
     * This function is used to set max limit for buffer distace textbox on buffer unit change
     */
    _checkAndSetLimitsForBufferDistance: function () {
      var rangeMessage;
      var bufferDistanceUnit = this.bufferDistanceUnit.value;
      switch (bufferDistanceUnit) {
        case "UNIT_STATUTE_MILE":
          this.bufferDistanceTextBox.set("constraints", {
            "min": 0,
            "max": 1000
          });
          rangeMessage = string.substitute(this.nls.bufferDistanceAndUnit.bufferRangeMessage,
            {
              "distance": 1000,
              "unit": this.nls.units.miles
            });
          this.bufferDistanceTextBox.set('rangeMessage', rangeMessage);
          break;
        case "UNIT_KILOMETER":
          this.bufferDistanceTextBox.set("constraints", {
            "min": 0,
          ...
          rangeMessage = string.substitute(this.nls.bufferDistanceAndUnit.bufferRangeMessage,
            {
              "distance": 1609.344,
              "unit": this.nls.units.kilometers
            });
          this.bufferDistanceTextBox.set('rangeMessage', rangeMessage);
          break;
        case "UNIT_FOOT":
    ...
    /**
     * This function is used to check whether buffer distance value is exceeding max limits as per 
     * selected unit or not if distance value exceeds max limit then
     * set max limit in buffer distance on blur
     */
    _checkAndSetValueForBufferDistance: function () {
      var bufferDistance = this.bufferDistanceTextBox.getValue();
      var selectedUnit = this.bufferDistanceUnit.value;
      var maxBufferDistanceByUnit = {
        "UNIT_STATUTE_MILE": 1000,
        "UNIT_KILOMETER": 1609.344,
        "UNIT_FOOT": 5280000,
        "UNIT_METER": 1609344
      };
      //if entered buffer value is greater than the max limit for the selected unit, set max limit in buffer distance
      if (maxBufferDistanceByUnit.hasOwnProperty(selectedUnit) &&
        bufferDistance > maxBufferDistanceByUnit[selectedUnit]) {
        this.bufferDistanceTextBox.set("value", maxBufferDistanceByUnit[selectedUnit]);
        this._allowFocusOnFirstNode = false;
        this._validateAndAddAOI();
      }
    }
